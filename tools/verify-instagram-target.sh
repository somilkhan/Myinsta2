...
for needle in "${checks[@]}"; do
  if grep -Fq "$needle" "$work/strings.txt"; then
    printf '%-6s %s\n' FOUND "$needle"
  else
    printf '%-6s %s\n' MISS "$needle"
    failed=1
...
grep -E 'DownloadOptionsBottomSheetFragment|DirectThreadFragment.saveMessageMedia|DirectThreadMediaSaver|accounts/set_biography/|profile_bio' "$work/strings.txt" | sort -u | head -100 || true
